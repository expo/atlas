...
import { createRequest, respond } from './http';

type ExpoRouterServerManifestV1Route<TRegex = string> = {
  page: string;
...
type ExpoRouterServerManifestV1FunctionRoute = ExpoRouterServerManifestV1Route<RegExp>;
...
export function createHttpRequestHandler(
  { build }: { build: string },
  setup?: Parameters<typeof createRequestHandler>[1]
): RequestHandler {
  const handleRequest = createRequestHandler(build, setup);
