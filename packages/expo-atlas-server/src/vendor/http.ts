...
import { createReadableStreamFromReadable, writeReadableStreamToWritable } from '../streams';
...
export function convertRequest(req: http.IncomingMessage, res: http.ServerResponse): Request {
  ...
  if (req.method !== 'GET' && req.method !== 'HEAD') {
    ...
    // @ts-expect-error - `duplex` is not in the RequestInit type, but should be
    init.duplex = 'half';
  }
...
export async function respond(res: http.ServerResponse, expoRes: Response): Promise<void> {
  ...
  // @ts-expect-error - `headers` is not in the ServerResponse type, but should be
